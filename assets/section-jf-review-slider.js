if (!customElements.get('jf-review-slider')) {
  class JfReviewSlider extends HTMLElement {
    ...
    measureTrack() {
      ...
      this.maxScrollLeft = Math.max(this.track.scrollWidth - this.track.clientWidth - 2, 0);
      this.updateButtons();
    }

    updateButtons() {
      const maxScrollLeft = typeof this.maxScrollLeft === 'number'
        ? this.maxScrollLeft
        : Math.max(this.track.scrollWidth - this.track.clientWidth - 2, 0);

      this.setButtonState(this.track.scrollLeft <= 2, this.track.scrollLeft >= maxScrollLeft);
    }

    setButtonState(disablePrev, disableNext) {
